...
var dfdSecureObjectDefaults = $.get(getActionUrl("GetSecureObjectDefaults", "Admin"))
    .done(function (data) {
        ...
        soVM.editor.secureObjectDefaults = _secureObjectDefaults;
    })
...
var dfdAuditTypes = $.get(getActionUrl("GetAuditTypes", "Admin"))
    .done(function (data) {
        _auditTypes = data;
        soVM.editor.auditTypes = _auditTypes;
    })
    .fail(function (jqXHR, textStatus, errorThrown) {
        let msg = decipherJqXhrError(jqXHR, textStatus);
        notifyError("There is a problem getting information from the server" + "<br/>" + msg);
...
function setChange(trueorfalse) {
    if (soVM.editor.hasChanges == trueorfalse) return;
    soVM.editor.hasChanges = trueorfalse;
}
function setError(trueorfalse) {
    if (soVM.editor.hasError == trueorfalse) return;
    soVM.editor.hasError = trueorfalse;
}
function setSelectedUId(uId) {
    if (soVM.selectedUId != uId) {
        soVM.selectedUId = uId;
    }
}
    ...
    verifySaveChanges().then(function (proceed) {
        if (proceed) {
            ...
            if ("#" + e.id == ID.SO_BTN_NEW) {
                soVM.editor.model.set("ParentUId", soVM.selectedUId);
            }
        }
    });
}
...
function saveSecureObject() {
    ...
    // clone the model
    var model = JSON.parse(JSON.stringify(soVM.editor.model));
...
function getTrusteeName(dataItem) {
    ...
    _kDsTrustees.filter({ operator: "eq", field: "UId", value: dataItem.TrusteeUId });
...
function createRightCheckBoxList(container, model) {
    ...
    container.find(".k-checkbox").bind('click', function (e) {
        var cb = $(e.target);
        var bitFlag1 = parseInt(cb.val() as string);
        ...
        if (cb.prop("checked")) {
            if (isCompositeValue) {
                ...
                container.find("input:checkbox:not(:checked)").each(function (index) {
                    var bitFlag2 = parseInt($(this).val() as string);
